...
if(request.session.tybe=="admin"){
        ...
        console.log(data);
        response.render("speaker/speakerlist.ejs", { speakers: data })
    })
        .catch((err) => { console.groupCollapsed(err + "") });
}else{
    request.flash('error','log in as admin to list all user')
    response.redirect("/login")
}
...
speakerrouter.get("/update/:_id?", function (request, response, next) {
    ...
    speakermodel.find(request.query)
        .then((data) => {
            // response.send(data)
            console.log(request.query._id, "dddddddddd", request.session.idval)
            if (request.query._id == request.session.idval||request.session.tybe=="admin") { response.render("speaker/speakeredit.ejs", data[0]); }
            else {
